...
class VID(data.Dataset):
    def __init__(self, args, is_cropped=False, root='data/vid', dstype='train', replicates = 1):
        ...
        self.img_pair_list = self._get_img_pair_list(dataset_split=dstype)
        ...
        assert (len(self.img_pair_list) == len(self.ano_pair_list))

    def _get_img_pair_list(self, dataset_split):
        _img_pair_list = []
        if dataset_split == 'train':
            data_path = os.path.join(self.root,
                                     'Data/VID',
                                     dataset_split)
            # data/VID/Data/VID/train/train_0/
            subsets = os.listdir(data_path)
            for _subset in subsets:
                _subset_path = os.path.join(data_path, _subset)

                # data/VID/Data/VID/train/train_0/train_0_vid_0/
        ...
        return _img_pair_list

    def _get_ano_list(self, dataset_split):
        _ano_pair_list = []
        if dataset_split == 'train':
            data_path = os.path.join(self.root,
                                     'Annotations/VID',
