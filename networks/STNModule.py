...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, spatial_dims, kernel_size, use_dropout=False):
        ...
        # [441, 45, 80]
        self.init_conv = nn.Conv2d(in_channels, 128, kernel_size=self._ksize, stride=1, bias=False)
        # [128, 45, 80]
        self.conv1 = nn.Conv2d(128, 256, kernel_size=self._ksize, stride=2, padding=1, bias=False)  # size : [1x3x32x32]
        # [256, 23, 40]
        self.conv2 = nn.Conv2d(256, 256, kernel_size=self._ksize, stride=1, padding=1, bias=False)
        # [256, 23, 40]
        self.conv3 = nn.Conv2d(256, 512, kernel_size=self._ksize, stride=2, padding=1, bias=False)
        # [512, 12, 20]
        self.conv4 = nn.Conv2d(512, 512, kernel_size=self._ksize, stride=1, padding=1, bias=False)
        # [512, 12, 20]
        self.reduce_conv = nn.Conv2d(512, 128, kernel_size=self._ksize, stride=1, padding=1, bias=False)
        # [128, 12, 20]

        self.fc1 = nn.Linear(20 * 12 * 128, 1024)
